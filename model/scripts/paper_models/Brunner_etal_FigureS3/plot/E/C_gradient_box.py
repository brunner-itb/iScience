...
for m,model in enumerate(models):
    ...
    for sv, scan_variable in enumerate(scan_variables):
        ...
        fc_that_worked = []
        fc_that_didnt_work = []
        for s, scan in enumerate(g_df[scan_variable].unique()):
            if len(g_df.loc[g_df[scan_variable] == scan, "replicat_index"].unique()) > 10:
                fc_that_worked.append(scan)
            else:
                fc_that_didnt_work.append(scan)
        g_df = g_df.loc[g_df[scan_variable].isin(fc_that_worked)]
...
scaled_df["gradient_standard"] = scaled_df.loc[scaled_df["fold_change"] == 1, "gradient"].mean()
scaled_df["gradient_change"] = (scaled_df["gradient"] - scaled_df["gradient_standard"]) # to pM
scaled_df.loc[scaled_df["gradient_change"] == -np.inf, "gradient_change"] = 0
...
# ax.set(ylabel=r"gradient (pM/µm)", xlabel="", ylim=ylim, yticks=[0., 0.15, 0.30])
ax.set(ylabel=r"gradient (pM/µm)", xlabel="", ylim=ylim, yticks=[0., 0.25, 0.50])
plt.xticks(xticks, [r"f$_{\rm sec}$", r"$\sigma$", r"K$_{\rm D}$"])
