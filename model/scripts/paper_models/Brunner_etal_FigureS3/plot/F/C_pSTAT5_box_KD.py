...
# define the standard values (fold-change = 1) for each of the scan_variables
standards = [7.43]
# standards = [1]
...
g1 = sns.barplot(data=scaled_df, y="activation", x="scan_variable", hue="fold_change", palette=["white", "grey", "black"],
                errwidth=1, linewidth=0.5, edgecolor="black", dodge=True, width=0.7)
ax.set(ylabel=r"pSTAT$^+$ (%)", xlabel="", ylim=(0,10))
plt.xticks([0.], [r"saturation"], rotation=-0, ha="left")
...
ax.set(ylabel=r"pSTAT$^+$ (%)", xlabel="", title=r"K$_{\rm D}$: saturation const.", ylim=(-1/6,6))
