...
if feedback_type == "pos":
    ...
    gamma = 12.
    fraction = 0
    filename = "pos_R_over_time_every_cell_and_means.pdf"
    x_ticks = [0, 2, 4]
    xlim = (None, None)
    ...
    fraction = 3
    path = "/{extra}/{u}/paper_models/kinetics/{mn}/{n}/".format(u=user, n=name, mn=model_name, extra=hdd)
    filename = "neg_R_over_time_every_cell_and_means.pdf"
    x_ticks = [0, 4, 8]
    xlim = (None, 8)
...
for plot_df in [cell_df]:
    ...
    #         104, 584,  44, 152, 796, 540, 117]) # hand-picked representative cell trajectories
    # elif feedback_type == "neg":
    #     picked_cell_ids = np.array([601, 345, 448, 613, 774, 380, 734, 661,  95, 637, 527, 303, 348,
    #        434, 226, 364, 350, 702, 103, 172, 573, 942, 793]) # hand-picked representative cell trajectories
    reduced_df = plot_df.loc[plot_df["id_id"].isin(picked_cell_ids)]

    sns.set(rc={"lines.linewidth": 0.2})
    # sns.lineplot(x="time", y="IL-2_R", data=reduced_df.loc[(reduced_df["type_name"] == "Th") & (~reduced_df["id"].isin(act_ids))].sort_values(by="time"), estimator=None,
    ...
    sns.lineplot(x="time", y="IL-2_R",
                        data=plot_df.loc[(plot_df["type_name"] == "Th") & (plot_df["activated"] == "pSTAT5$^-$")].sort_values(by="time"),
                      palette=palette, ci=0, legend=False, alpha=0.5)
    plt.xlabel("time (d)")
    plt.xscale(xscale)
    plt.xlim(xlim)
    plt.xticks(x_ticks)
