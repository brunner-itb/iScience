...
if feedback_type == "pos":
    ...
    gamma = 12.
    fraction = 0
    filename = "pos_pSTAT5_over_time_every_cell_and_means.pdf"
    x_ticks = [0, 2, 4]
    xlim = (None, None)
    ...
    fraction = 2
    path = "/{extra}/{u}/paper_models/kinetics/{mn}/{n}/".format(u=user, n=name, mn=model_name, extra=hdd)
    filename = "neg_pSTAT5_over_time_every_cell_and_means.pdf"
    x_ticks = [0, 4, 8]
    xlim = (None, 8)
...
for plot_df in [cell_df]:
    ...
    picked_cell_ids = np.random.choice(cell_ids, int(len(cell_ids) * picked_fraction * 2), replace=False)
    # if feedback_type == "pos":
    ...
    sns.lineplot(x="time", y="IL-2_pSTAT5", data=reduced_df.loc[(reduced_df["type_name"] == "Th") & (~reduced_df["id"].isin(act_ids))].sort_values(by="time"), estimator=None,
    ...
    sns.lineplot(x="time", y="IL-2_pSTAT5",
    ...
    if save_plot == True:
        fig.savefig(saving_string + "means_" + filename, bbox_inches='tight', transparent=True, dpi=1200)
    plt.tight_layout()
    plt.show()