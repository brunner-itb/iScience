...
def ODE(cells):
    ...
    c = dr.molecules_to_molar(
        I_lin(p["R_Th"], p["q"], p["f_Tsec"], p["f_Treg"], p["k_on"], p["R_Tsec"], p["R_Treg"], p["N_cells"],
              p["kd"])) * 1e12
    return c

def analytical(cells):
    p["N_cells"] = cells
    p["f_Tsec"] = 1 / cells
    c = ODEdr.molecules_to_molar(high_density_concentration(p, p["R_Th"], int(p["N_cells"] * (1 - p["f_Treg"] - p["f_Tsec"])))) * 1e12
    return c
...
fig, ax = plt.subplots()
sns.lineplot(data=cell_df, x = "cells", y = "IL-2_surf_c", label="RD-system", color="#e4c137fd", errorbar="se",
             marker="o", markersize = 2, markeredgecolor="#e4c137fd")
# plt.axhline(ODEdr.molecules_to_molar(high_density_concentration(p, p["R_Th"])) * 1e12, 0, 1, color="black", label="analytical")
plt.plot(cells, [ODE(value) for value in cells], color="black", label="ODE", marker="o", markersize = 2)
plt.plot(cells, [analytical(value) for value in cells], color="green", linestyle="--", label="analytical solution", marker="o", markersize = 2)
# ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.xlabel("system size (# cells)")
plt.ylabel("surface conc. avg. (pM)")
plt.xscale("log")
plt.yscale("linear")
...
fig.savefig(fig_path + "FigS1_mesh_convergence_surf_c.pdf", bbox_inches='tight', transparent=True)
plt.tight_layout()
plt.show()
...
sns.set_theme(context="talk", style="ticks", rc=rc_ticks)
fig, ax = plt.subplots()
cells = cells_from_x_grid(cell_df.geometry_x_grid.unique(), margin, L)[1:]
...
for c, cell in enumerate(cells):
    ...
    for r, rep in enumerate(c_df.replicat_index.unique()):
        ...
        rep_RD.append(np.mean(rep_df["IL-2_surf_c"] - analytical_sol[c]))
...
plt.fill_between(cells, RD - RD_std, RD + RD_std, color="#e4c137fd", alpha=0.3)
...
fig.savefig(fig_path + "FigS1_mesh_convergence_diff_to_analytical.pdf", bbox_inches='tight', transparent=True)
plt.tight_layout()
plt.show()
