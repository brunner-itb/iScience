...
saving_string = r"/home/brunner/Documents/Current work/2023_11_03/boxplot_static_activation_homogen.pdf"
...
for q in np.sort(svs):
    ...
    homo_boxplot_data.append(np.array(act_list_2) * 100)
...
plt.plot(ODE_c_df["IL-2_q"].unique()[:change_idx + 1], np.mean(ODE_boxplot_data, axis=1)[:change_idx + 1], color=color2, linewidth=0.6)
plt.scatter(ODE_c_df["IL-2_q"].unique()[change_idx], np.mean(ODE_boxplot_data, axis=1)[change_idx], marker = "o",
            edgecolor=color2, s=5, color="white", linewidth=0.5)

plt.plot(ODE_c_df["IL-2_q"].unique()[change_idx + 1:], np.mean(ODE_boxplot_data, axis=1)[change_idx + 1:], color=color2, linewidth=0.6)
plt.scatter(ODE_c_df["IL-2_q"].unique()[change_idx + 1], np.mean(ODE_boxplot_data, axis=1)[change_idx + 1], marker = "o",
            edgecolor=color2, s=5, color=color2, linewidth=0.5)
